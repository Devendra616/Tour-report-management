...
exports.login = (req, res) => {
  ...
  db.query("SELECT * FROM admins WHERE sap_id = ?", [sap_id], (err, rows) => {
    ...
    const token = jwt.sign(
      { id: admin.id, sap_id: admin.sap_id, role: "admin" },
      process.env.JWT_SECRET,
      { expiresIn: "8h" }
    );

    res.json({ token, admin: { id: admin.id, sap_id: admin.sap_id } });
  });
};
